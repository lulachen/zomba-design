...
powNfn<- function(N,nsims,thetau,nblocks){
  ...
  powfn2 <- function(obsZ,y0,tau,cov1,cov2,blocks){
    # y0 is assumed y0/sd(y0)
    # covs are assumed mean centered
    y1 <- y0S+tau
    ## Z <- sample(obsZ) ## complete_ra(N=N)
  ...
  theps<-replicate(nsims,powfn2(obsZ=theZ,y0=y0S,tau=thetau,cov1=healthMDb,cov2=incomeMDb,blocks=blocks))
...
powNfn(N=100,nsims=10,thetau=.5,nblocks=3)
